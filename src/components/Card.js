    ...
    return (
        <div className="card">
            ...
            <div className="item-2">
                <h2>{movie.title}</h2>
                {movie.release_date ? <h5>Sorie le : {dateFormater(movie.release_date)}</h5> : ""}
                <h4>{movie.vote_average}/10 <span>⭐</span></h4>
                <ul>{movie.genre_ids ? genreFinder() : movie.genres.map((genre, index) => <li
                    key={index}>{genre.name}</li>)}
                </ul>
